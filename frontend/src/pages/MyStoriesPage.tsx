    ...
    <div className="min-h-screen bg-heritage-800">
      ...
      <div className="px-4 pb-4 max-w-6xl mx-auto">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-serif text-heritage-100">My Stories</h1>
          {canManage && (
            <Link
              to="/submit-story"
      ...
      <div className="px-4 pb-10 max-w-6xl mx-auto">
        {loading ? (
          ...
          <div className="text-heritage-100/80">You haven't posted any stories yet.</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((s) => (
              <div key={s.id} className="bg-white rounded-lg shadow border border-brand-sage/30 overflow-hidden">
                <div className="p-4">
                  <h3 className="text-lg font-semibold text-heritage-800 line-clamp-2">{s.title}</h3>
                  <div className="mt-2 text-sm text-heritage-800/70 line-clamp-3">{(s.excerpt || s.body || s.content || '').toString()}</div>
                  ...
                  <div className="mt-4 flex gap-2">
                    {canManage && (
                      <>
                        <button onClick={() => onEdit(s)} className="px-3 py-1 text-sm rounded bg-heritage-500 text-heritage-800 hover:bg-heritage-500/90">Edit</button>
                        <button onClick={() => onDelete(s)} className="px-3 py-1 text-sm rounded border border-error/30 text-error-dark hover:bg-error/10">Delete</button>
                      </>
                    )}
                  </div>
                </div>
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white w-full max-w-lg rounded-lg shadow-lg p-6">
            <h2 className="text-xl font-semibold text-heritage-800 mb-4">Edit Story</h2>
